...
train_pipeline = [
    dict(type='LoadImageWithDepthFromFile', multiple_camera_param=True),
    ...
    dict(type='Collect', keys=['img', 'depth', 'gt_semantic_seg', 'gt_masks', 'gt_labels', 'gt_nutrition', 'gt_ingredients'])
]
...
model = dict(
    ...
    #     train_std=train_std,
    #     loss_reg=dict(type='MultiTaskUncertaintyLoss'),
    #     # loss_reg=dict(type='LogL1Loss', loss_weight=2.0),
    # ), 
    regression_head=None,
    text_encoder=dict(
        type='PredefinedOvClassifier',
        clip_model_name="ViT-L-14",
        pretrained='/data/zengyuzhi/project/nutrition/pretrained/open_clip_pytorch_model.bin',
        cache_feature=True,
        templates=["a photo of food containing {}."],
    ), 
...
optimizer = dict(
    ...
    paramwise_cfg=dict(
        custom_keys={
            ...
            'visual_proj_branch': dict(lr_mult=1.),
            'decode_head': dict(lr_mult=1.), 
            'regression_head': dict(lr_mult=0.0)
        }
    )
)
...
checkpoint_config = dict(by_epoch=False, interval=2500, max_keep_ckpts=3)
...
log_level = 'INFO'
# load_from = '/data/zengyuzhi/project/nutrition/work_dirs/CLIP_segment/TTTT/iter_30000.pth'
load_from = None
resume_from = None
workflow = [('train', 1)]
