...
class ViTAdapter(TIMMVisionTransformer):
    def __init__(self, pretrain_size=224, num_heads=12, conv_inplane=64, n_points=4,
                 ...
                 use_extra_extractor=True, with_cp=False, with_depth=None, num_plate_emd=20, *args, **kwargs):
        ...
        self.spm_depth = SpatialPriorModule(in_channels=1, kernel_size=5, inplanes=conv_inplane, embed_dim=embed_dim, with_cp=False)

        assert with_depth is None or len(with_depth) == len(interaction_indexes) 
        f"with_depth should have the same length as interaction_indexes, but got {len(with_depth)} and {len(interaction_indexes)}"

        if with_depth is None:
            with_depth = [False] * len(interaction_indexes)
        ...
        self.interactions = nn.Sequential(*[
            InteractionBlock(dim=embed_dim, num_heads=deform_num_heads, n_points=n_points,
                             ...
                             with_cp=with_cp, with_depth=self.with_depth[i], num_plate_emd=num_plate_emd)
    ...
    def forward(self, x, depth): 
        ...
        c, d = torch.cat([plate_embed, c2, c3, c4], dim=1), torch.cat([d2, d3, d4], dim=1)
        ...
        for i, layer in enumerate(self.interactions):
            ...
            x, c = layer(x, c, d, self.blocks[indexes[0]:indexes[-1] + 1],
                         deform_inputs1, deform_inputs2, H, W)
            outs.append(x.transpose(1, 2).view(bs, dim, H, W).contiguous())
        
        # Split & Reshape
        plate_embed = c[:, 0:self.num_plate_emd, :]
        c2 = c[:, self.num_plate_emd:self.num_plate_emd + c2.size(1), :]
        c3 = c[:, self.num_plate_emd + c2.size(1):self.num_plate_emd + c2.size(1) + c3.size(1), :]
        c4 = c[:, self.num_plate_emd + c2.size(1) + c3.size(1):, :]
        ...
        f3 = self.norm3(c3)
        f4 = self.norm4(c4)
        return [plate_embed, f1, f2, f3, f4]
